...
import (
	...
	storesdk "cosmossdk.io/store/types"
...
type Source struct {
	...
	Logger     log.Logger
	Cms        storesdk.CommitMultiStore
}

// NewSource returns a new Source instance
func NewSource(home string, encodingConfig *params.EncodingConfig) (*Source, error) {
	levelDB, err := db.NewGoLevelDB("application", path.Join(home, "data"), nil)
	if err != nil {
		return nil, err
	}
	...
	return &Source{
		...
		Logger:     log.NewLogger(os.Stdout).With("module", "explorer"),
...
func (k Source) LoadHeight(height int64) (sdk.Context, error) {
	var err error
	var cms storesdk.CacheMultiStore
	if height > 0 {
