...
func NewNode(config *Details, txConfig client.TxConfig, codec codec.Codec) (*Node, error) {
	...
	dbProvider := cmtconfig.DefaultDBProvider
	...
	csMetrics, _, _, smMetrics, _, _, _ := metricsProvider(genDoc.ChainID)

	evidenceDB, err := dbProvider(&cmtconfig.DBContext{ID: "evidence", Config: tmCfg})
	if err != nil {
		return nil, err
	}
...
func initDBs(config *cfg.Config, dbProvider cmtconfig.DBProvider) (blockStore *store.BlockStore, stateDB dbm.DB, err error) {
	var blockStoreDB dbm.DB
	blockStoreDB, err = dbProvider(&cmtconfig.DBContext{ID: "blockstore", Config: config})
	if err != nil {
		return
	}
	blockStore = store.NewBlockStore(blockStoreDB)

	stateDB, err = dbProvider(&cmtconfig.DBContext{ID: "state", Config: config})
	if err != nil {
		return
	}
...
func createAndStartIndexerService(
	config *cfg.Config,
	dbProvider cmtconfig.DBProvider,
	eventBus *tmtypes.EventBus,
	logger log.Logger,
	...
	case "kv":
		store, err := dbProvider(&cmtconfig.DBContext{ID: "tx_index", Config: config})
		if err != nil {
