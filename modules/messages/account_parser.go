...
func DefaultMessagesParser(cdc codec.Codec, cosmosMsg sdk.Msg) ([]string, error) {
	msgSigners, _, err := cdc.GetMsgV1Signers(cosmosMsg)
	if err != nil {
		return nil, err
	}
	var signers = make([]string, len(msgSigners))
	for index, signer := range msgSigners {
		signers[index] = sdk.AccAddress(signer).String()
	}
	return signers, nil
}
